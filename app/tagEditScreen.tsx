...
export default function TagEditScreen() {
  const { selectedTags: initialSelectedTags } = useLocalSearchParams<{ selectedTags: string }>();
  const [selectedTags, setSelectedTags] = useState<string[]>(initialSelectedTags ? JSON.parse(initialSelectedTags) : []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        router.setParams({ updatedTags: JSON.stringify(selectedTags) });
        router.back();
      } else {
    ...
    <ScreenWrapper>
      <View style={styles.container}>
        ...
        <View style={styles.footer}>
          ...
          <TouchableOpacity style={styles.saveButton} onPress={handleSave} disabled={isLoading || selectedTags.length < MIN_TAGS}>
